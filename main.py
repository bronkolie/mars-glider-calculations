...
for airfoil in airfoils:
    with open(os.path.join("airfoils", airfoil.filename)) as airfoil_csv:
        alpha_cl = []
        alpha_cd = {}
    ...
    for needed_cl in lift_coefficient:
        min_alpha = 100
        for alpha, cl in alpha_cl:
            if alpha < min_alpha and cl >= needed_cl:
                min_alpha = alpha
        angle_of_attack.append(min_alpha)
        if min_alpha != 100:
            zero_lift_drag_coefficient.append(alpha_cd[min_alpha])
            display.append(True)
        else:
    ...
    glide_distance = glide_ratio * begin_altitude / 1000 # km
    glide_pythagorean_distance = np.sqrt((glide_distance * 1000) ** 2 + begin_altitude ** 2) # m
    ...
    plots[0].plot(velocity, glide_distance, label=airfoil.name)
